...
export class DinoGame {
  ...
  private getDinoHitbox(): Rect {
    return {
      ...
      bottom:
        this.dino.position.y + DINO_HEIGHT * (1 - DINO_HITBOX_INSET_BOTTOM),
    }
  }
  ...
  private getCactusHitbox(): Rect {
    return {
      left:
        this.cactus.position.x + this.cactus.width * CACTUS_HITBOX_INSET_LEFT,
      right:
        this.cactus.position.x +
        this.cactus.width * (1 - CACTUS_HITBOX_INSET_RIGHT),
      top:
        this.cactus.position.y + this.cactus.height * CACTUS_HITBOX_INSET_TOP,
      bottom:
        this.cactus.position.y +
        this.cactus.height * (1 - CACTUS_HITBOX_INSET_BOTTOM),
    }
  }
  ...
  private renderOverlay(showStartHint: boolean) {
    ...
    if (this.isGameOver) {
      ...
      ctx.fillText(
        ...
        this.width / 2,
        this.height / 2 + 24 * UI_TEXT_SCALE
      )
      ctx.textAlign = 'start'
      return
    }
  ...
  private updateRunAnimation(delta: number) {
    ...
    while (this.runAnimationElapsed >= frameDuration) {
      ...
      this.runFrameCursor =
        (this.runFrameCursor + 1) % DINO_RUN_FRAME_INDICES.length
    }
  }
  ...
  private renderDino() {
    ...
    const sourceY =
      DINO_SPRITE_FIRST_FRAME_Y + frameIndex * DINO_SPRITE_FRAME_STRIDE_Y
